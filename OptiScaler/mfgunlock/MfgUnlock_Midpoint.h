 ...
 *
 * Ported into OptiScaler from the RenoDX "MFG Unlock" addon (mfgunlock/midpoint.hpp).
 * The algorithm is unchanged; only the namespace and the string formatting follow
 * OptiScaler conventions. No ReShade dependency remains.
 *
 * ---------------------------------------------------------------------------
 * THE PROBLEM
 *
...
inline constexpr TemporalProfile kTemporalProfiles[] = {
...
inline bool ReadRelativePointer(const uint8_t* base, size_t imageSize, const uint8_t* slot, ptrdiff_t displacement,
                                uint64_t& value)
{
...
inline bool Apply(HMODULE module, std::vector<Patch>& patches, void*& allocation, std::string& detail)
{
    ...
    size_t fatSize = 0;
    const Internal::TemporalProfile* selectedProfile = nullptr;
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i, ++section)
    {
        ...
        for (size_t off = 0; off + sizeof(uint64_t) <= size; off += sizeof(uint64_t))
        {
            ...
            if (Internal::ReadU32(candidate) != Internal::kFatbinMagic)
                continue;

            const Internal::TemporalProfile* nameProfile = nullptr;
            for (const auto& profile : Internal::kTemporalProfiles)
            {
                uint64_t entryName = 0;
                uint64_t descName = 0;
                if (!Internal::ReadRelativePointer(base, imageSize, sec + off, profile.entryNameOffset, entryName) ||
                    !Internal::ReadRelativePointer(base, imageSize, sec + off, profile.descriptorNameOffset, descName))
                {
                    continue;
                }
                if (Internal::PointsToCString(base, imageSize, entryName, profile.entryName) &&
                    Internal::PointsToCString(base, imageSize, descName, profile.descriptorName))
                {
                    nameProfile = &profile;
                    break;
                }
            ...
            if (total < 1024 || total > (16u << 20))
                continue;
            if (value + total > start + imageSize)
                continue;
            const auto* fatProfile = Internal::FindTemporalProfile(candidate, total);
    ...
    std::string why;
    if (!Internal::BuildTemporalFatbin(fat, fatSize, *selectedProfile, rebuilt, why))
    {
        detail = why;
        return false;
    }
    ...
    detail = std::format("redirected {} {} descriptor(s) from a {}-byte fatbin to a {}-byte temporal-corrected rebuild",
                         patches.size(), selectedProfile->descriptorName, fatSize, rebuilt.size());
    return true;
}
...
inline void Restore(std::vector<Patch>& patches, void*& allocation)
{
    ...
    {
        DWORD oldProtect = 0;
        if (VirtualProtect(patch.slot, sizeof(uint64_t), PAGE_READWRITE, &oldProtect) == 0)
            continue;
        *patch.slot = patch.original;
        DWORD ignored = 0;
        VirtualProtect(patch.slot, sizeof(uint64_t), oldProtect, &ignored);
    }
    patches.clear();
