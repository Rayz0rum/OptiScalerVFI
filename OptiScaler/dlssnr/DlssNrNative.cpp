...
void AllocateSession(Session&s,Device&d,ID3D12GraphicsCommandList*c){if(s.allocated)return;auto dev=d.owner.Get();auto&uploadBatch=*new UploadBatch(dev);c=uploadBatch.cmd.Get();const uint64_t P=((s.tokens+127)/128)*128;
 if(S().candidate&&s.tokens==960)s.partials=Allocate(dev,4ull*960*1024*2);s.xp=Allocate(dev,P*1024/2);s.xs=Allocate(dev,P*1024/16);s.yp=Allocate(dev,P*4096/2);s.ys=Allocate(dev,P*4096/16);s.c=Allocate(dev,P*1024*2);if(!d.contracts.count(s.tokens)){std::string have;for(auto&kv:d.contracts){if(!have.empty())have+=", ";have+=std::to_string(kv.first);}throw std::runtime_error("no contract for M="+std::to_string(s.tokens)+" (this resolution); vendor assets cover M="+have);}s.cw=Allocate(dev,d.contracts.at(s.tokens).workspaceBytes);
